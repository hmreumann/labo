...
dtrain  <- fread("./datasets/paquete_premium_202011_ext.csv")

#genero el modelo,  aqui se construye el arbol
modelo  <- rpart("clase_ternaria ~ .",  #quiero predecir clase_ternaria a partir de el resto de las variables
                 data = dtrain,
                 xval=0,
                 cp=  -0.759926335594352,   #esto significa no limitar la complejidad de los splits
                 minsplit=  1448,     #minima cantidad de registros para que se haga el split
                 minbucket=  563,     #tamaño minimo de una hoja
...
fwrite( entrega, 
        file= "./labo/exp/KA2001/K101_rpart_fe_bo.csv", 
        sep= "," )